/**
 * StegoChat (Alice's side)
 * Receives messages from Bob
 */
...
#define SERVERPORT "20077"	// the port users will be connecting to
...
int main(int argc, char *argv[])
{
	...
	struct sockaddr_in bob; // Struct for Bob's contact info
	...
	while (keep_communicating == 1) {// Do we still communicate?
		...
		for (index = 0; index < strlen(message); index++) { // For each char in message
			...
			if ((numbytes = sendto(fd, passage, strlen(passage), 0, (struct sockaddr*)&bob, sizeof(bob))) == -1) {
				perror("alice: sendto (in progress)");
				exit(1);
			}
